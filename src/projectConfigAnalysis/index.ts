...
function getFilesPath(include: string[], exclude: string[]): string[] {
  ...
  const includeDirPaths: string[] = include.map(path =>
    join(currentPath, path)
  );
  const exlcudeDirPaths: string[] = exclude.map(path =>
    join(currentPath, path)
  );
...
export default function() {
  ...
  const includeConfig = ((config.include as string[]) ?? ['']).map(item =>
    item.replace(/(\/\**\/\*)|(\/\*)/g, '')
  );
  const excludeConfig = ((config.exclude as string[]) ?? []).map(item =>
    item.replace(/(\/\**\/\*)|(\/\*)/g, '')
  );
